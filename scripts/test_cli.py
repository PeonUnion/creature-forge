#!/usr/bin/env python3
"""CreatureForge CLI 流程化测试（数据隔离，不污染真实 data/）。

覆盖物种 / 动作 / 预设 / 渲染 的完整生命周期（创建→验证→修改→删除），
通过 subprocess 调用真实 CLI 进程（python -m creatureforge.cli），断言退出码与输出。
...
def cli(*args: str) -> str:
    ...
    r = subprocess.run(
        [PY, "-m", "creatureforge.cli", "--data-dir", str(DATA_DIR), *args],
        capture_output=True, text=True, cwd=REPO,
    )
    if r.returncode != 0:
        raise AssertionError(
            f"CLI 失败: creatureforge.cli {' '.join(args)}\n"
            f"--- stdout ---\n{r.stdout}\n--- stderr ---\n{r.stderr}"
        )
    return r.stdout
...
def _species_create(sid: str, title: str = "CLI 测试物种") -> dict:
    return {
        "species_id": sid, "schema": "creatureforge_species_v1", "title": title,
...
def _action_create(aid: str, title: str = "CLI 测试动作") -> dict:
    return {
        "schema": "creatureforge_motion3d_v1", "motion_id": aid, "title": title,
...
def _preset_create(pid: str, title: str = "CLI 测试预设", body: dict | None = None) -> dict:
    return {
        "schema": "creatureforge_preset_v1", "preset_id": pid, "species": "human",
...
class TestCliWorkflow(unittest.TestCase):
    ...
    def test_preset_crud(self):
        ...
        self.assertEqual(d["schema"], "creatureforge_preset_v1")
        self.assertEqual(d["species"], "human")
